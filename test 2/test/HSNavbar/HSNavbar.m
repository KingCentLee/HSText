...
@implementation HSNavbar
@synthesize stateBarColor = _stateBarColor;
@synthesize stateBarStyle = _stateBarStyle;

- (void)drawRect:(CGRect)rect
{
    self.backgroundColor = UIColorFromRGB(0x333333);  //[UIColor blackColor];   //UIColorFromRGB(0x215caa);
...
- (void)setNeedsLayout
{
    ...
    /**< 起到在IOS 7中navbar 和stats bar 不 悬浮的作用*/
    self.translucent = NO;
...
- (void)setImage:(NSString *)image
{
    ...
    [_button setBackgroundImage:image_ forState:UIControlStateNormal];
    [_button setBackgroundImage:image_ forState:UIControlStateHighlighted];
    [_button setBackgroundImage:image_ forState:UIControlStateSelected];
}
...
-(void)setNewTitleImageTitle:(UIImage *)image title:(NSString *)title
{
    ...
    view.frame = CGRectMake(0, 0, 180, 20);
    view.backgroundColor = [UIColor clearColor];
    UIImageView *imageView = [[UIImageView alloc] initWithImage:image];
    imageView.frame = CGRectMake(20, 2, 35, 18);
    UILabel *label = [[UILabel alloc] init];
    label.frame = CGRectMake(36, 0, 144, 20);
    label.backgroundColor = [UIColor clearColor];
    //    label.tag = 99901;
...
-(void)setLogoImageTitle:(NSString *)title
{
    ...
    view.frame = CGRectMake(0, 0, 180, 20);
    view.backgroundColor = [UIColor clearColor];
    UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"logoXX"]];
    imageView.frame = CGRectMake(20, 2, 34, 16);
    UILabel *label = [[UILabel alloc] init];
    label.frame = CGRectMake(36, 0, 144, 20);
    label.backgroundColor = [UIColor clearColor];
    //    label.tag = 99901;
                     ...
                     backType:(id) obj
{
    ...
    [backBtn setImage:[UIImage imageNamed:@"topleft"] forState:UIControlStateNormal];//top_return_index
    [backBtn setImage:[UIImage imageNamed:@"topleft"] forState:UIControlStateHighlighted];//top_return_index
    UIBarButtonItem *leftItem = [[UIBarButtonItem alloc] initWithCustomView: backBtn];
    UIBarButtonItem *negativeSpacer = [[UIBarButtonItem alloc]
                                       initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace
                                       target:nil action:nil];
    if(IOS_VERSION >= 7.0)
        negativeSpacer.width = 14-19;// margin-19  ios7
    else
        negativeSpacer.width = 14-8; // margin-8 ios6
    
    //不同：
    backBtn.frame = CGRectMake(0, 0, 10, 20);
    [backBtn addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
    
    [self setLeftBarButtonItems:[NSArray arrayWithObjects:negativeSpacer, leftItem, nil] animated:NO];
}
                    ...
                    closeType:(id) obj
{
    //相同：
    UIButton *backBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [backBtn setImage:[UIImage imageNamed:@"close"] forState:UIControlStateNormal];//top_return_index
    [backBtn setImage:[UIImage imageNamed:@"close"] forState:UIControlStateHighlighted];//top_return_index
    UIBarButtonItem *leftItem = [[UIBarButtonItem alloc] initWithCustomView: backBtn];
    UIBarButtonItem *negativeSpacer = [[UIBarButtonItem alloc]
                                       initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace
                                       target:nil action:nil];
    ...
    //不同：
    backBtn.frame = CGRectMake(0, 0, 21, 22);
    [backBtn addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
                       ...
                       backType:(id) obj withTitle:(NSString *)title
{
    ...
    [titleBtn setImage:[UIImage imageNamed:@"jiantou2"] forState:UIControlStateNormal];//top_return_index
    [titleBtn setTitle:title forState:UIControlStateNormal];
    [titleBtn setImageEdgeInsets:UIEdgeInsetsMake(0, 90, 0, 0)];
    [titleBtn setTitleEdgeInsets:UIEdgeInsetsMake(0, -10, 0, 0)];
    //不同：
    titleBtn.frame = CGRectMake(0, 0, 100, 20);
    titleBtn.tag = 1001;
...
{
    NavBarButtonItem *buttonItem = [NavBarButtonItem backItemWithTarget:nil
                                                                 action:nil
                                                                  title:@""];
    [buttonItem setItemType:NavBarButtonItemTypeDefault];
    self.leftBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:buttonItem.button];
}
                            ...
                            image:(NSString *)image
{
    ...
    buttonItem.button.frame = CGRectMake(0, 0, 21, 22);
